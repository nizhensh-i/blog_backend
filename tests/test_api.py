...
class TestApiCase:
    ...
    def test_posts(self, client, auth):
        ...
        auth.login()
        # 发布文章
        r = client.post('/api/v1/posts/', headers= auth.get_headers(), json={'body': '666'})
        assert r.json.get('body') == '666'
    ...
    def test_comments(self,client, auth):
        auth.register()
        auth.login()


        c = client.get('/api/v1/42/comments/', headers=auth.get_headers())
        assert r.json.get('data')

    def test_search_followed(self, client, auth):
        auth.register()
        auth.login()

        c = client.get('/api/v1/search_followed?name=11', headers=auth.get_headers())
        assert c.json.get('data')
